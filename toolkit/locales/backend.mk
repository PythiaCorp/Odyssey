...
$(MDDEPDIR)/update.locale.stub: /workspace/Odyssey/src/toolkit/locales/generate_update_locale.py $(if $(IS_LANGUAGE_REPACK),FORCE)
	$(REPORT_BUILD)
	$(call py_action,file_generate,--locale=$(AB_CD) /workspace/Odyssey/src/toolkit/locales/generate_update_locale.py main update.locale $(MDDEPDIR)/update.locale.pp $(MDDEPDIR)/update.locale.stub)
	@$(TOUCH) $@

MISC_TARGETS += $(MDDEPDIR)/locale.ini.stub
locale.ini: $(MDDEPDIR)/locale.ini.stub ;
EXTRA_MDDEPEND_FILES += $(MDDEPDIR)/locale.ini.pp
$(MDDEPDIR)/locale.ini.stub: /workspace/Odyssey/src/toolkit/locales/generate_locale_ini.py $(if $(IS_LANGUAGE_REPACK),FORCE)
	$(REPORT_BUILD)
	$(call py_action,file_generate,--locale=$(AB_CD) /workspace/Odyssey/src/toolkit/locales/generate_locale_ini.py main locale.ini $(MDDEPDIR)/locale.ini.pp $(MDDEPDIR)/locale.ini.stub)
	@$(TOUCH) $@
...
JAR_MANIFEST := /workspace/Odyssey/src/toolkit/locales/jar.mn
